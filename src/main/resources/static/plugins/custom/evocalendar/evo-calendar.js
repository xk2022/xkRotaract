...
;(function(factory) {
    ...
    EvoCalendar.prototype.buildSidebarYear = function() {
        ...
        // 更新側邊欄顯示的年份     
        _.$elements.sidebarEl.find('.calendar-year > p').text(displayedYears[0] + "-" + displayedYears[1]);
    };
    ...
    EvoCalendar.prototype.buildTheBones = function() {
        ...
        if (!_.$elements.calendarEl.html()) {
            ...
            markup = '<div class="row calendar-headbar">';

                // 電腦版 row district-list
                markup += '<div class="district-list">';
                for (var i = 0; i < headerTabs.length; i++) {
                    var activeClass = (i === 0) ? ' active' : ''; // 設置第一個為 active
                    markup += '<div class="col"><div class="btn tab-btn' + activeClass + '">' + headerTabs[i] + '</div></div>';
                }
                markup += '</div>';

                // 手機版 row district-select
                markup += '<div class="district-select px-7"><select class="form-select form-control form-control-solid" id="inputGroupSelect_district">';
                for (var i = 0; i < headerTabs.length; i++) {
                    var activeClass = (i === 0) ? ' selected' : ''; // 設置第一個為 active
                    markup += '<option value="' + headerTabs[i] + activeClass + '">' + headerTabs[i] + '</option>'
                }
                markup += '</select></div>';
            ...
            markup += '<div class="calendar-sidebar">'+
                        ...
                        '<ul class="calendar-months">';
                            for(var i = 0; i < _.$label.months.length; i++) {
                                ...
                                markup += '<li class="month" role="button" data-month-val="'+_.$activeR.num+'">'+_.initials.dates[_.options.language].months[i]+'</li>';
